...
def train_and_validate(cfg, model, LFdataset, device, logger, filtered_data=None, batch_per_epoch=None):
    ...
    for snap_num in range(5):
        ...
        batch_id = 0
        use_old_centers = snap_num > 0 
        model.current_snaps = snap_num
        for epoch in range(train_epoch[snap_num]):
            model.EntityModel.adjuster_text.cached_clusters = None  
            model.EntityModel.adjuster_img.cached_clusters = None
            ...
            for batch in islice(train_loader, batch_per_epoch):
                ...
                pred, reloss = parallel_model(LFdataset.snapshots[snap_num].ent_graph, batch, relation_inv,  use_old_centers=use_old_centers)
            ...
            result = test(cfg, snap_num, snap_num, model, LFdataset, relation_inv, use_old_centers, device=device, logger=logger, t_v_data = 'valid')
        ...
        for test_num in range(snap_num+1):
            ...
            result = test(cfg, test_num, test_num, model, LFdataset, relation_inv, use_old_centers, device=device, logger=logger, t_v_data = 'test')
...
@torch.no_grad()
def test(cfg, test_num, snap_num, model, LFdataset, relation_idv, use_old_centers, device, logger,  t_v_data, return_metrics=False):
    world_size = util.get_world_size()
    rank = util.get_rank()
    if t_v_data == 'valid':
    ...
    for batch in test_loader:
        ...
        t_pred, _ = model(LFdataset.snapshots[snap_num].ent_graph_all, t_batch, relation_idv, use_old_centers)
        h_pred, _ = model(LFdataset.snapshots[snap_num].ent_graph_all, h_batch, relation_idv, use_old_centers)

...
class LifeMKG(nn.Module):
    ...
    def __init__(self, device, dataset,  entity_model_cfg):
        ...
        self.adjuster_text = self.EntityModel.adjuster_text
        self.adjuster_img = self.EntityModel.adjuster_img
        self.hper=[0, 0, 0, 0.001, 0.005]
        self.current_snaps = -1
        for name, param in self.named_parameters():
            self.register_buffer(('old.' + name).replace(".", "_"), torch.tensor([[]]))
        
    def forward(self, entity_graph_data, relation_change, batch, r_ind, use_old_centers):

        text_feature,  img_feature = self.GetRepresentations(entity_graph_data, use_old_centers)
        score = self.EntityModel(entity_graph_data, batch, r_ind, 0, self.RelEmbeddings.weight.data, text_feature, img_feature, use_old_centers)
    ...
    def switchsnaps(self, index):
        ...
        for name, param in self.named_parameters():
            ...
            if "RelationProjectionWeight1" in name:
                RelationProjectionWeight1 = nn.Parameter(torch.empty((self.rel_nums[index+1], 128, 64))).to(self.device) 
                ...
                NewRelationProjectionWeight1[:self.rel_nums[index]] = torch.nn.Parameter(value)
                self.EntityModel.layers[num].RelationProjectionWeight1 = torch.nn.Parameter(NewRelationProjectionWeight1)

            if "RelationProjectionBias1" in name:
                RelationProjectionBias1 = nn.Parameter(torch.empty((self.rel_nums[index+1], 1, 64))).to(self.device) 
                nn.init.uniform_(RelationProjectionBias1, -1/math.sqrt(64*2), 1 / math.sqrt(64*2))
                NewRelationProjectionBias1 = RelationProjectionBias1.data
                NewRelationProjectionBias1[:self.rel_nums[index]] = torch.nn.Parameter(value)
                self.EntityModel.layers[num].RelationProjectionBias1 = torch.nn.Parameter(NewRelationProjectionBias1)
                num += 1
...
class Entity(BaseNBFNet):
    ...
    def __init__(self, input_dim, hidden_dims, num_relation=1, **kwargs):
        ...
        for i in range(len(self.dims) - 1):
            self.layers.append(
                self.layers.GeneralizedRelationalConv( 
    ...
    def forward(self, data,  batch, r_ind,  rel_embeddings, text_feature, img_feature, use_old_centers):
        ...
        text_feature = self.adjuster_text(text_feature, feature_all, use_old_centers)
        img_feature= self.adjuster_img(img_feature, feature_all, use_old_centers)
        
        index = t_index.unsqueeze(-1).expand(-1, -1, feature.shape[-1])

        feature = text_feature.gather(1, index) 
        feature_all = img_feature.gather(1, index) 
...
class ClusterCentersAdjuster(torch.nn.Module):
    ...
    def forward(self, features, feature_all, use_old_centers=False):
        features = self.vis2sem(features)

        device = features.device


        if self.cached_clusters is None or self.cached_cluster_centers is None:
            features_np = features.cpu().detach().numpy()

      
            if use_old_centers and self.cached_cluster_centers is not None:
                init_centers = self.cached_cluster_centers.cpu().detach().numpy()
                kmeans = KMeans(
                    n_clusters=self.cached_cluster_centers.shape[0], 
                    random_state=0,
                    n_init=1,  
                    init=init_centers  
                )
            else:
                kmeans = KMeans(n_clusters=100, random_state=0, n_init='auto', init='k-means++')
        ...
        batch_size = feature_all.shape[0]
        cluster_centers_features = torch.matmul(cluster_centers[clusters], self.weight)
        cluster_centers_features = cluster_centers_features.unsqueeze(0).expand(batch_size, -1, -1)
        adjusted_features = feature_all * cluster_centers_features
